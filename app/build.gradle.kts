...
// Grok API key from local.properties
...
android {
    ...
    signingConfigs {
        create("release") {
            ...
            storePassword = System.getenv("RELEASE_STORE_PASSWORD")?.takeIf { it.isNotBlank() }
                ?: localProperties.getProperty("RELEASE_STORE_PASSWORD", "")
            keyAlias = System.getenv("RELEASE_KEY_ALIAS")?.takeIf { it.isNotBlank() }
...
dependencies {
    ...
    // Retrofit + OkHttp (for Open Food Facts API)
    ...
    // ML Kit Text Recognition (OCR for receipt scanning)
    implementation("com.google.mlkit:text-recognition:16.0.1")

    // ML Kit Document Scanner (edge detection, deskew, contrast for receipts)
    ...
    // FuzzyWuzzy (fuzzy string matching for shopping list auto-strike)
